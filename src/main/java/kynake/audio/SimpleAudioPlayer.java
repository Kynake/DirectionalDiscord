...
public class SimpleAudioPlayer implements AudioPlayer, Runnable {
  ...
  private Queue<short[]> pcmBuffer = new ConcurrentLinkedQueue<>();
  ...
  public void run() throws IllegalStateException {
    ...
    while(audioLine.isOpen()) { // Keep the Thread open while we process audio
      short[] nextSample =  pcmBuffer.poll();
      if(nextSample != null) {
        // Add next buffered sample to audioLine
        audioLine.write(Utils.shortToByteArray(nextSample, AudioReceiveHandler.OUTPUT_FORMAT.isBigEndian()), 0, nextSample.length);
